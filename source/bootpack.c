...
void HariMain(void)
{
	...
	sheet_updown(sht_mouse, 2); //鼠标位于窗口下
	...
	load_tr(3 * 8);
	task_b_esp = memman_alloc_4k(memman, 64 * 1024) + 64 * 1024 - 8; //任务B的栈 64K - 参数占用
	tss_b.eip = (int)&task_b_main; //tss_b 内容，切换后CPU会读取
	...
	tss_b.fs = 1 * 8;
	tss_b.gs = 1 * 8;
	*((int*)(task_b_esp + 4)) = (int)sht_back; //将sht_back 存入内存
	mt_init();
