...
void HariMain(void)
{
	...
	struct SHEET *sht_back, *sht_mouse, *sht_win, *sht_cons; //3个b窗口
	struct TASK *task_a, *task_cons; // 任务
	struct TIMER *timer;
	static char keytable0[0x80] = { //shift未按下
	...
	static char keytable1[0x80] = { //shift按下
		0,   0,   '!', 0x22, '#', '$', '%', '&', 0x27, '(', ')', '~', '=', '~', 0,   0,
		'Q', 'W', 'E', 'R', 'T', 'Y', 'U', 'I', 'O', 'P', '`', '{', 0,   0,   'A', 'S',
		'D', 'F', 'G', 'H', 'J', 'K', 'L', '+', '*', 0,   0,   '}', 'Z', 'X', 'C', 'V',
		'B', 'N', 'M', '<', '>', '?', 0,   '*', 0,   ' ', 0,   0,   0,   0,   0,   0,
	...
	*((int *)(task_cons->tss.esp + 4)) = (int)sht_cons; //第一个参数
	*((int *)(task_cons->tss.esp + 8)) = memtotal;		//第二个参数
	task_run(task_cons, 2, 2); //不同优先级
	...
	sheet_updown(sht_mouse, 3); //鼠标位于最上层
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i <= 511) {    //键盘
				...
				if (i == 256 + 0x36) {	/* 右sheft ON */
					key_shift |= 2;
				}
				if (i == 256 + 0xaa) {	/* 左sheft OFF */
					key_shift &= ~1;
				}
